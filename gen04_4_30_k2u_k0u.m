...
% 22.08.2014 11:16
clear all;clc;
KU=[ 0 0.05 0.1 0.15 0.2 ];
AKU=[ 1 1.157894737 1.333333333 1.529411765 1.750000000 ];
...
In=I_c;
fn=f0;
VAL_MATRIX=[     Un   AKU(k)*Un       Un      0  ; 
                 0 -2*pi/3  +2*pi/3      0  ;
                 In      In       In      0  ; 
                 0       0        0      0 ];
GEN=generator(fs,fn,duration,VAL_MATRIX);
GEN=GEN.generate;
GEN=GEN.savefile(['signal\' num2str(KU(k),'%2.2f') ' K2U_K0U\test_state_1.txt']);
clear GEN;
...
GEN=GEN.savefile(['signal\' num2str(KU(k),'%2.2f') ' K2U_K0U\test_state_2.txt']);
...
GEN=GEN.add_interharmonics(IHARM_MATRIX);
GEN=GEN.savefile(['signal\' num2str(KU(k),'%2.2f') ' K2U_K0U\test_state_3.txt']);
clear GEN;
end;
